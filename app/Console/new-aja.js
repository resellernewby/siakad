...
if (
  window.mR.findModule("ChatCollection")[0] &&
  window.mR.findModule("ChatCollection")[0].ChatCollection
) {
  if (
    typeof window.mR.findModule("ChatCollection")[0].ChatCollection.findImpl ===
      "undefined" &&
    typeof window.mR.findModule("ChatCollection")[0].ChatCollection._find !=
      "undefined"
  ) {
    window.mR.findModule("ChatCollection")[0].ChatCollection.findImpl =
      window.mR.findModule("ChatCollection")[0].ChatCollection._find;
  }
}
...
window.WWebJS.sendMessage = async (chat, content, options = {}) => {
  ...
  if (options.attachment) {
    ...
    // tambahan agar ada caption pada gambar
    attOptions.caption = content;
...
/* == Tambahan function/method == */
window.WWebJS.checkNumberStatus = async (id) => {
  try {
    const isValid = await Store.QueryExist({ type: "phone", phone: id });
...
window.WWebJS.sendMediaMessage = async (
  ...
  try {
    ...
    const msg = await window.WWebJS.sendMessage(chat, caption, options);
    return true;
    // return { msg: msg, status: true };
  } catch (error) {
    return false;
...
window.WWebJS.getAllGroups = async (done) => {
  const groups = await window.Store.Chat.filter((chat) => chat.isGroup).map(
...
window.WWebJS.getGroupParticipantIDs = async (groupId, done) => {
  const a = await window.Store.GroupMetadata.find(groupId);

  const output = await a.participants._models.map((data) => ({
    phone: data.id.user,
    name:
      data.contact.name ||
      data.contact.verifiedName ||
...
window.WWebJS.getAllContacts = async (done) => {
  const contacts = await Store.Contact._models
    .filter((t) => t.id.server == "c.us")
    .map((data) => ({
      phone: data.id.user,
      name: data.name || data.verifiedName || data.pushname || data.id.user,
    }));
